...
while True:
        ...
        if data.find('public_key')!=-1: #client has sent their public key
            ...
            for i in range(0, 8):
                (data, addr) = mySocket.recvfrom(SIZE)
                decryptedCharacter = decrypt(pk, int(data))            
                des_keyA.append(decryptedCharacter)
            des_key = "".join(des_keyA)
            ...
            ###################################your code goes here####################
            newDes=des.des()
            #stringData = "".join(map(chr, data))
            
            decryptedData=newDes.decrypt(des_key, data)
            print("Decrypted Data")
            #print(rr_byte)
            
            rr_byte = bytes(decryptedData, "ISO-8859-1")
            print(rr_byte)
            #print(rr_byte)
            ...
            #rr_byte=bytearray(rr_byte)
            #print(type(rr_byte))
            #rr_byte = bytearray(rr_byte, 'utf-8')
            #print(type(rr_byte))

            #intArr=[]
            #for i in rr_byte:
            #     intArr.append(ord(i))
            #rr_byte=bytearray(intArr)
            #print(rr_byte)
